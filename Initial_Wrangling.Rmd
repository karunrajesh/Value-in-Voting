---
title: "Value in Voting"
author: "Karun Rajesh, Han Truong, Davy Deng"
output:
  html_document:
    df_print: paged
---
...
## Initial Questions
Our dataset includes historical data on how much money each administration spent or proposed to spend on different sectors, historical voter turnout by states, and the winning margin in historical presidential election by states. As a voter who cares about a particular sector, how much money is my singular vote corresponding to the spending in that sector, given the chances of my vote flipping the result of a presidential election based on the state that I live in?

Furthermore, is there a relationship between the voter turnout in a state and the margin of winning in a state in past presidential elections? 

Lastly, using the designation of "swing state" defined by the narrow winning margins, can we predict what states will most likely to be a key competitive swing state in the 2024 presidential election using the historical data on the margin of the winning party? Is this competitiveness classification correlated with voter turnout in each state?
...
# Aggregate Senate by state
senate_turnout <- senate %>%
  group_by(year, state) %>%
  filter(row_number()==1) %>%
  dplyr::select(year, state, totalvotes)

# Aggregate Presidential Election by state
...
margin_pres <- presidential %>%
  ...
  filter(row_number() %in% c(1,2)) %>% # Get top 2 candidates
  dplyr::select(year, state, candidate, candidatevotes, totalvotes, party) %>%
  summarize(margin = mean((candidatevotes[1]-candidatevotes[2])/totalvotes), party = party[1]) #Take difference between candidates and divide by total votes
...
parties_pres <- presidential %>%
  group_by(year,state) %>%
  arrange(desc(candidatevotes), .by_group = TRUE) %>%
  filter(row_number() == 1) %>% # Get the winner for the given state
...
# Aggregate Demographics by state
context_state<- context %>%
  group_by(state) %>%
  dplyr::select(-county) %>% # Get rid of county variable
  summarise_each(funs(mean)) # Get mean within each county
```
...
budg_all <- data.frame(lapply(budg_all, as.character), stringsAsFactors=FALSE) # Convert factor columns to character
...
budg = as.data.frame(lapply(budg_all, function(x) as.numeric(gsub(",","",x)))) # Take out comma in numerical values
colnames(budg) <- cols
rownames(budg) <- rows
budg <- budg[,-c(1,2)]
budg_admin <- as.data.frame(sapply(seq(1,dim(budg)[2]-1,by=4),function(i) rowSums(budg[,i:(i+3)], na.rm=TRUE))) # Aggregate each of the president's discretionary budget in their four years of presidency
...
The United States Election Project is an information source that houses election data for each state over the past 50 years of elections. This source was mainly used to obtain the voter turnout as a percentage of the total eligible voters. As can be seen, we do not have data from the 1976 and 2016 election. This dataset was combined with two other sources that were web scraped to obtain the full voter turnout per state from the 1976 to 2016 presidential election. 

```{r Turnout}
check <- read.csv("Data/Turnout.csv")
check <- check %>%
  filter(X %in% seq(1976, 2016, by = 4)) %>%
  filter(State != "United States") %>% # Remove United States from the dataset
  select(X, State, X.4)
colnames(check) <- c("Year", "State", "Turnout")
check$Year <- as.numeric(as.character(check$Year))
total_turnout <- check
total_turnout$Turnout <- as.numeric(gsub("%","",total_turnout$Turnout)) # Remove the % in turnout
head(total_turnout)
...
# The table was an image, so we had to manually input all values
# We obtained all state abbreviations with state.abb
...
We web scraped several other datasets to fill in gaps found throughout the process. One of these was the voter turnout from 2016. This was obtained by scraping a table from sos.nh.gov that contained voter turnout in the year 2016. Various other datasets that showed differences in administration’s proposed budgets were also web scraped from https://www.thebalance.com/ . This site contains the breakdown of discretionary spending per year, and contains tables comparing President Trump to President Obama’s (2012) proposed budgets. We also scraped data from govinfo.gov that contained data comparing President Bush’s and President Obama’s (2008) proposed budget. We realized that it would not be efficient to download each year as a separate web scraped table, so we replaced these with the White House OMB data. 


```{r Web Scraping}
# Web Scraping, NOT USED SINCE OMB HAS SAME INFORMATION

# Clean table obtained from web scraping
...
# Compares the Obama and Trump Discretionary Budget Proposals
...
# Extract the table
out <- extract_tables(location) # Since pdf, need to use extract_tables
df <- out[[4]]
...
final_f <- mutate_all(final, function(x) as.numeric(as.character(x))) #Convert factors to characters then to numeric values
rownames(final_f) = str_match(rows, "^(.*?)\\.")[,2] # Get the department names from regular expression
head(final_f)
```

In order to find the full voter turnout from 1976 to 2016 presidential elections, we had to web scrape the voter turnout for the 2016 election. We then manually input the data for the 1976 presidential election voter turnout. 

```{r }
# Get the 2016 table from the pdf
...
final <- data.frame(lapply(final, as.character), stringsAsFactors=FALSE) # Conver factor to character
colnames(final) <- final[1,]
final <- final[-1,]

final <- final[,c(11,12)]
final <- final %>%
  filter(`2016` != "United States") # Remove United States from the dataset
final$X <- 2016
final <- final[,c(3,1,2)]
colnames(final) <- c("Year", "State", "Turnout")
final$Year <- as.numeric(final$Year)

write.csv(final,"Data/2016_Turnout.csv",row.names=F) #Write csv to the Data folder

# Manually put in the 1976 voter turnout, since could not parse the file 
...
total_turnout <- rbind(total_turnout, old_turnout) #Append the dataframes together


# how we are reading turnout in shiny app, 
...
#Append the dataframes fully
pr_turnout <- turnout_1976 %>% 
...
Here we show the demographics for the 2016 election:

```{r Demographics}
# Demographics, 2016 Election
...
presidential_turnout %>%
  ...
  ggtitle("Voter Turnout in Virginia") +
  ylab("Voter Turnout") +
  geom_line() +
  geom_point()
...
presidential_turnout %>%
  filter(state == "Virginia") %>% # Show example for one state
...
margin_pres %>% 
  ...
  ggtitle("Margin of Victory in Virginia") +
...
```{r echo=FALSE}
us_states <- map_data("state") %>% mutate(state=region) # Get the map of the US with each state
...
#Setup the scales for the margins and voter turnout
...
mapdata <- presidential_turnout_w_map %>% filter(year==2016) %>% mutate(fillcolor=turnout) # Map the turnout to the heat map
...
```{r echo=FALSE}
mapdata<- margin_pres_w_map %>% filter(year==2016) %>% mutate(fillcolor=margin) # Map the margins to the heat map
...
```{r echo=FALSE}
mapdata <- parties_pres_w_map %>% filter(year==2016) %>% mutate(fillcolor=str_to_title(party)) # Map the eletion winners to the heat map
...
```{r }
# Chance of influencing the election, based on 2012 results
# Show 5-6 states
...
```{r}
# Show the discretionary spending for the given government agencies
...
First we filter by state and find the probability of one vote turning an election for that state:
...
```{r }
# Show the value of a vote for the given agencies
...
```{r BubblePlot, message=FALSE, warning=FALSE, out.height='100%', out.width='100%'}
### Relationship between  over Time
...
p
```
For 2016 presidential election, there is definite negative correlation between voters turnout and vote margin results for each state. We will now fit a logistic regression model with linear covariates on historical data for which states were competitive in past presidential elections. The covariates in our regression include US Census variables for these states as well as the election year and voters turnout data for these states. We use election data starting from 2000, as we did not believe that data before this point was representative of possible swing states.

```{r Regression, fig.asp=0.95, fig.width=5, warning=FALSE}
# Put in Census variables as features
# Put in vote margins
state_name_to_abb <- setNames(state.name,state.abb)
full_set <- margin_pres %>%  
...
set_in <- full_set %>% 
  filter(year >= 2000) %>%
  select(-margin, - party, - white_pct) %>% unique()
...
We achieve 0.9 for the AUC of our ROC Curve, which means our classifier fitted pretty well onto the dataset. The US Census variables were not as helpful and neither was the year. Now we can forecast into 2024 on which state will be competitive.
...
The goal of this project was to understand more about the landscape of voting and how much it impacts us. Our initial exlporatory analysis allowed us to understand more about the nuances within each presidenital election like voter turnout, margins of victory, demographics, and results of each election over the last 40 years. This gave us, and hopefully the user, how elections have changed over time and where they are trending. 

We then moved onto the value of a single vote. This started with understanding the probability of a single vote overturning an election. We showed this through Andrew Gelman's probability table that gave such a probability for each state. This was then coupled with the discretionary budget throughout a president's term to find the value in a single vote. We hoped to communicate that voting can be viewed as more than just a civic duty, and how much impact it had in our lives. By illustrating that a single vote in Virginia resulted in expected value of $35,000 just for one agency, we start to understand just how much value and power comes with a vote. We also encouraged users to find out more about the value of a vote and trends in voting with our Shiny app. 

We finally extended our analysis looking forward to the 2024 election, and seeing which state is expected to be the most competitive. By finding the most competitive state, the value of the vote in that state drastically increases exponentially. Our model showed that Florida and Maine were expected to be the most competive states in the 2024 election. This does not mean that just competitive states are the only ones worth voting in. There are several other levels of elections (senate, district) that were not looked at, where this same type of analysis can be applied. A vote holds so much value on the national, regional, and local level. We should take pride in the (literal) amount our vote can bring.
