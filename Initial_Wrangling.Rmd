...
```{r}
example <- budg_admin %>%
  filter(rownames(budg_admin) %in% c("Department of Education", "Department of Transportation"))
...
example %>%
  ggplot(aes(x=as.numeric(year), y=value, color=variable)) + geom_line() + geom_point() +
  xlab("Year") + ylab("Discretionary Spending Amount")
