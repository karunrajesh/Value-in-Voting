...
p <- scatter_data %>%
  ...
  scale_color_manual(name='Winning Part',values=c("Democrat"="blue","Republican"="red","Democratic-Farmer-Labor"="darkgreen")) +
  ...
  ggtitle(sprintf("Turnout vs Margin in %d", year_scatter)) +
  theme_bw() 
...
set_in <- full_set %>% select(-sector, - value) %>% unique()
swing_clf<- glm(swing_state ~ . - swing_state - state -margin - party - white_pct,family=binomial(),data=set_in)
summary(swing_clf)
...
roc_df %>% arrange(desc(sensitivity),desc(specificity))
roc_df %>% ggplot(aes(specificity,sensitivity)) + geom_line(color='darkred') + geom_abline(aes(slope=1,intercept=1),linetype='dashed') + xlim(1,0)
